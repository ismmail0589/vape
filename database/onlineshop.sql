...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 28, 2023 at 02:27 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `getcat` (IN `cid` INT)   BEGIN
  SELECT
    *
  FROM
    `categories`
...
(1, 'Muhammad Ismail', 'Ismail', 'ismail999972@gmail.com', 'ismail123', '0310060662', 'Royal Avenue, Meherban Colony Chatta Bakhtawar, Islamabad, Islamabad Capital Territory', 'Nafees Boys'),
(2, 'Muhammad Ismail', 'Ismail', 'ismail9972@gmail.com', 'ismail123', '0310060662', 'Royal Avenue, Meherban Colony Chatta Bakhtawar, Islamabad, Islamabad Capital Territory', 'Nafees Boys');

--
-- Triggers `user_info`
--
DELIMITER $$
CREATE TRIGGER `after_user_info_insert` AFTER INSERT ON `user_info` FOR EACH ROW BEGIN INSERT INTO `user_info_backup` VALUES (
    NEW.`user_id`,
    NEW.`first_name`,
    NEW.`last_name`,
    NEW.`email`,
